...
void ClockSim::Run() {
  ...
  std::unique_ptr<Matrix> rotation = Matrix::RotationZMatrix(pi_6);
  ...
  std::unique_ptr<Tuple> rotated_p;
  for (int i = 1; i < 12; i++) {
    rotation = Matrix::RotationZMatrix(i * pi_6);
    rotated_p = *rotation * (*point);
    rotated_p = *rotated_p + *center;
